...
def calc_average_packets(num_packets):
    total_packets = [val["Total Protocol Packets"] for key, val in num_packets.items() if "Total Protocol Packets" in val]
...
def main(args):
    ...
    np = df.plot.bar(y="Total Packets", color=["lightskyblue", "lightgreen", "turquoise", "palegreen"], rot=0, ylim=(0, 55000), 
                     title="Escenario " + escenario + ": Media de paquetes por protocolo", legend=False)
